...
class PermissionDependency:
    ...
    async def __call__(
        ...
        if (
            self.allow_override and 
            current_user.role == Role.SUPER_ADMIN and 
            override_role == "true"
        ):
            return current_user
...
class Permissions:
    ...
    PENTESTER_WITH_OVERRIDE = PermissionDependency(
        allowed_roles={Role.PENTESTER}, 
        allow_override=True
    )
    
    # Allow pentesters and clients
    PENTESTER_AND_CLIENT = PermissionDependency(
        allowed_roles={Role.PENTESTER, Role.CLIENT},
        allow_override=True
    )
    ...
    @staticmethod
    def custom_roles(roles: List[Role], allow_override: bool = True) -> PermissionDependency:
        """Create a custom permission dependency with specific roles"""
        return PermissionDependency(allowed_roles=set(roles), allow_override=allow_override) 