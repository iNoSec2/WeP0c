...
async def create_project(
    ...
    """Create a new project with the pentester as a lead and the specified client"""
    return crud_project.create_project(db=db, project=project, pentester_id=current_user.id)
...
async def read_projects(
    ...
    if status:
        ...
        if current_user.role == Role.PENTESTER:
            return [p for p in projects if current_user.id in [pentester.id for pentester in p.pentesters]]
        ...
        if current_user.role == Role.PENTESTER:
            return crud_project.get_pentester_projects(db, pentester_id=current_user.id, skip=skip, limit=limit)
        elif current_user.role == Role.CLIENT:
            return crud_project.get_client_projects(db, client_id=current_user.id, skip=skip, limit=limit)
...
async def read_project(
    ...
    elif current_user.role == Role.PENTESTER and current_user.id not in [p.id for p in project.pentesters]:
        raise HTTPException(status_code=403, detail="Not authorized to access this project")
...
async def update_project_details(
    ...
    return crud_project.update_project(db, project_id=project_id, project_data=project_update)
...
async def add_pentester_to_project(
    ...
    db: Session = Depends(get_db),
    current_user = Depends(require_role(Role.PENTESTER))  # Only existing pentesters can add others
):
    ...
    return crud_project.add_pentester_to_project(
        ...
        pentester_id=pentester_data.pentester_id
    )
...
async def remove_pentester_from_project(
    ...
    if len(project.pentesters) <= 1:
        raise HTTPException(status_code=400, detail="Cannot remove the last pentester from a project")
